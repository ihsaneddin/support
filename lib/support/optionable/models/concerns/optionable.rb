        ...
        module Optionable
          ...
          def _initialize_options
            if _options_field.present?
              ...
              if send(_options_column).nil?
                send("#{_options_column}=", _options_field)
              end
          ...
          def _options_field
            send(_options_column).blank?? self.class._options_mapping[:fields] : send(_options_column).deep_symbolize_keys rescue nil
          end
          ...
          module ClassMethods
            ...
            def has_options opts={}
              ...
              column_type = columns.select{|col| col.name == opts[:column].to_s }.first.try(:type)
              if column_type == :json
                serialize opts[:column].to_sym
              else
                serialize opts[:column].to_sym, JSON
              end
